...
import { SubcriptionPengembalianId } from "../../graphql/typeDefs/pengembalian.graphql";
import PengembalianListItemUser from "./PengembalianListItemUser";

const PengembalianListUser = ({uid}) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [page, setPage] = useState(1);
  const [limit, setLimit] = useState(10);

  const { data, loading } = useSubscription(SubcriptionPengembalianId, {
    variables: {
      id_users: uid,
        ...
        <div className="relative w-full overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
          <table className="whitespace-no-wrap w-full text-left text-sm text-gray-500">
            ...
            {data?.pengembalian_aggregate.nodes.map((item, i) => {
              return <PengembalianListItemUser key={item.id} data={{ ...item, no: i + 1 }} />;
            })}
          </table>
        </div>
      ) : (
...
export default PengembalianListUser;
