            ...
            {role === "admin" && (
              <>
                {isAuthorizedAdmin.map((item, idx) => {
                  const isActive = window.location.pathname.includes(item.path.replace(":uid", uid));
                  return (
                    <li key={idx}>
                      <NavLink
                      ...
                      {isActive && item.submenu && (
                        <ul className="pl-4">
                          {item.submenu.map((subItem, subIdx) => {
                            const isSubItemActive = window.location.pathname.includes(subItem.path.replace(":uid", uid));
                            return (
                              <li key={subIdx}>
                                <NavLink
