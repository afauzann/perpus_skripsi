...
import { SubscriptionKategori } from "../../graphql/typeDefs/kategori.graphql";
...
const KategoriList = () => {
  ...
  const { data, loading } = useSubscription(SubscriptionKategori, {
    variables: {
  ...
  return (
    <div className="container mx-auto py-6 px-6">
      ...
      {loading ? (
      ...
      ) : data?.kategori_aggregate.nodes.length > 0 ? (
        <>
        <div className="relative w-full overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
          <table className="whitespace-no-wrap w-full text-left text-sm text-gray-500">
            ...
            {data?.kategori_aggregate.nodes.map((item, i) => {
              return <KategoriListItem key={item.id} data={{ ...item, no: i + 1 }} />;
            })}
          </table>
        </div>
        ...
        <button
          ...
          disabled={data?.kategori_aggregate.nodes.length < limit}
          className="py-2 px-4 border border-gray-300 rounded-lg hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
        >
          Next
        </button>
