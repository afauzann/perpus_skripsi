...
import { SubscriptionKategoriCreate } from "../../graphql/typeDefs/kategori.graphql";

const ModalCreateBuku = ({ handleModalCreateTrigger }) => {
  const [kategoriSearchResults, setKategoriSearchResults] = useState([]);
  ...
  const { data: dataKategori, loading: loadingKategori } = useSubscription(
    SubscriptionKategoriCreate,
    {
  ...
  const handleKategoriSearch = (e) => {
    const keyword = e.target.value;
    setSearchTerm(keyword);
  };

  const handleKategoriSelect = (kategori) => {
    setData({ ...data, kategori_id: kategori.id });
    setSearchTerm(kategori.nama_kategori);
    setKategoriSearchResults([]);
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (data.nama_buku !== "" && data.gambar_buku !== null && data.kategori_id !== null) {
          ...
          <div className="relative w-full max-w-xs">
            ...
            >
              <div className="flex items-center justify-between rounded-t border-b p-4 dark:border-gray-600">
                <h3 className="p-1.5 text-xl font-semibold text-gray-900 dark:text-white">
                  Tambah Buku
                </h3>
                <button
                  type="button"
              ...
              <div className="space-y-6 p-6">
                ...
                <div>
                  ...
                  {loadingKategori ? (
                    ...
                    <div>
                      {dataKategori &&
                        dataKategori.kategori_aggregate.nodes.map(
                          (kategori) => (
                            <div
                              key={kategori.id}
